...
import { StatusEvento, IParticipanteSoloEvento, IParticipanteDuplaEvento, IParticipanteTimeEvento, IEventoResponse } from "./evento.type.js";
...
class ListarEventosAdapter {
    ...
    async execute(): Promise<IEventoResponse[]> {
        try {
            ...
            const result: IEventoResponse[] = response.map((data) => {
                return {
