...
import { StatusEvento, IParticipanteSoloEvento, IParticipanteDuplaEvento, IParticipanteTimeEvento, IEventoRequest, IEventoResponse } from "./evento.type.js";

class CriarEventoAdapter {
    statusMapper(status: unknown): StatusEvento {
        switch (status) {
            case 'aberto':
    ...
    eventoTipoMapper(data: unknown): IParticipanteSoloEvento | IParticipanteDuplaEvento | IParticipanteTimeEvento {
        let result: any;
    ...
    async execute (data: IEventoRequest): Promise<IEventoResponse> {
        ...
        try {
            const response = await Evento.create(payload);

            if (!response) {
                throw new Error("Campeonato não encontrado");
            }

            const result: IEventoResponse = {
...
export default CriarEventoAdapter;